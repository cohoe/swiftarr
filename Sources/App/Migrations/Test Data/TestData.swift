...
struct CreateTestData: AsyncMigration {
	...
	func createTestLargeForumPosts(on database: Database) async throws {
		let users = try await User.query(on: database).filter(\.$username ~~ ["james", "heidi", "sam", "verified"]).all()
	...
	func createTestLargeSeamailThread(on database: Database) async throws {
		...
		try await bigFez.$participants.attach(users, on: database, { $0.readCount = 0; $0.hiddenCount = 0 })
		for index in 0...824 {
			var postStr: String
			switch Int.random(in: 1...10) {
				case 1: postStr = "First Post!"
			...
			let post = try FezPost(fez: bigFez, authorID: users.randomElement()!.requireID(), text: "Post #\(index): \(postStr)", image: nil) 
			try await post.save(on: database)
		}
