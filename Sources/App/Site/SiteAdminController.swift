...
// MARK: - Admin Pages
	...
	func announcementCreatePostHandler(_ req: Request) async throws -> HTTPStatus {
		...
		try await apiQuery(req, endpoint: "/notification/announcement/create", method: .POST, encodeContent: postContent)
		return .created
	}
	...
	func announcementEditPostHandler(_ req: Request) async throws -> HTTPStatus {
		...
		let postContent = AnnouncementCreateData(text: text, displayUntil: displayUntilDate)
		try await apiQuery(req, endpoint: "/notification/announcement/\(announcementID)/edit", method: .POST, encodeContent: postContent)
		return .created
	}
	...
	func announcementDeletePostHandler(_ req: Request) async throws -> HTTPStatus {
		...
		let response = try await apiQuery(req, endpoint: "/notification/announcement/\(announcementID)", method: .DELETE)
	...
	func dailyThemeEditPostHandler(_ req: Request) async throws -> HTTPStatus {
		...
		let postContent = DailyThemeUploadData(title: postStruct.forumTitle ?? "", info: postStruct.postText ?? "", 
				image: ImageUploadData(postStruct.serverPhoto1, postStruct.localPhoto1), cruiseDay: postStruct.cruiseDay ?? 0)
		try await apiQuery(req, endpoint: "/admin/dailytheme/\(themeID)/edit", method: .POST, encodeContent: postContent)
		return .created
	}
	...
	func scheduleVerifyPostHandler(_ req: Request) async throws -> HTTPStatus {
		...
		try await apiQuery(req, endpoint: "/admin/schedule/update/apply?\(components.percentEncodedQuery ?? "")", method: .POST)
		return .ok
	}
